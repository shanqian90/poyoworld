...
export default function WorkRequestForm({
  ...
  const estimate = useMemo(() => {
    ...
    return { workTotal, deliveryTotal, total: workTotal + deliveryTotal, qtySum };
  }, [products, baseUnit, options.shipAgent]);
    ...
    <div className="border-t border-emerald-800/10 pt-3">
      ...
      <div className="bg-emerald-800/5 border border-emerald-300 rounded-xl px-3 py-3 mb-4">
        ...
        <div className="text-[11px] text-neutral-500 mt-1">
          ...
          {" · "}세금계산서/입금대행 관련 정확한 금액은 관리자 확인 후 별도 안내됩니다
        </div>
      </div>
...
function Field({ label, children }: { label: string; children: React.ReactNode }) {
  return (
    <div>
      <label className="block text-[11px] font-bold text-neutral-500 mb-0.5">{label}</label>
      {children}
    </div>
  );
}
