...
export async function GET() {
  ...
  const [bl, wl] = await Promise.all([
    supabase.from("blacklist").select("id, value, reason").eq("type", "name").order("created_at", { ascending: false }),
    supabase.from("whitelist").select("id, value, note").eq("type", "name").order("created_at", { ascending: false }),
  ]);
