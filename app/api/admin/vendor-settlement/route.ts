...
export async function GET(req: NextRequest) {
  ...
  const [requestsRes, paymentsRes] = await Promise.all([
    supabase
      .from("work_requests")
      .select("id, start_date, receipt_no, company_code, company_name, issue_date, deposit_amount, memo, tax_bill, status")
  ...
  const totalRequested = (requestsRes.data || []).reduce((sum, r) => sum + (r.deposit_amount || 0), 0);
...
export async function POST(req: NextRequest) {
  ...
  const { data, error } = await supabase
    .from("vendor_payments")
    .insert({ login_id: loginId, amount, paid_date: paidDate, memo })
    .select()
    .single();
